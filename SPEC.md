...
1. Require `--author "Name <email>"` and `--message`; accept optional repeated `--co-author "Name <email>"`. Validate identities and reject embedded control characters before publish startup, including during dry runs. Append co-author trailers after a blank line or extend an existing trailer block, preserving message text and avoiding duplicate co-author entries. Then run publish startup
...
4. Read the absorbed baseline from `origin/pubgate/public-approved:.pubgate-absorbed`; create or update branch `pubgate/publish` based on this absorbed commit, replacing all content with the current content of `origin/pubgate/public-approved`. Use the supplied public identity for both Git author and committer. Use the supplied message with any co-author trailers as the public commit message; its first line is the subject and the remaining body may contain standard trailers. Never fall back to internal Git configuration or generate a public message. Disable commit signing so an internal signing key cannot leak. Internal stage commits are logged for the operator but never embedded in the public commit or PR
